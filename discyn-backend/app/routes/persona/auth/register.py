...
async def register(persona: PersonaUsuariAdmin, session: AsyncSession = Depends(get_async_session)):
    ...
    if persona.rol == "usuari":
        # Solo validamos la altura, ya que pes fue eliminado
        valid_height = persona.altura is None or persona.altura > 59
        no_licence = persona.llicencia is None

        if no_licence and valid_height:
            user_db = UsuariDB(
                persona_id=persona_db.id,
                altura=persona.altura
                # pes ya no existe, por lo que no se pasa aquí
            )
            session.add(user_db)
        else:
            raise HTTPException(
                status_code=400,
                detail="The user must have a valid height (>59cm) or None, and must not have a license."
            )

    elif persona.rol == "administrador":
        if persona.altura is None and persona.llicencia is not None:
            ...
            raise HTTPException(status_code=400, detail="The administrator must have a valid license and no height")
